...
def get_cpu_info():
    ...
    """
        O primeiro chamado de psutil.cpu_percent geralmente retorna 0% porque ele usa a diferença 
        entre duas leituras para calcular o uso da CPU. Precisamos chamar uma vez antes da medição real.

        Assim como o comando top do Linux, cuja a primeira leitura pode não ser precisa, pois compara valores 
        consecutivos.
    """
    psutil.cpu_percent(interval=0.1, percpu=True)

    """
        Loop para evitar retornar apenas valores 0%. Isso acontece porque se a CPU estiver inativa no exato
        momento da medição, pode retornar 0% em todos os núcleos, o que não reflete o uso real.
    """
    cpu_usage = [0] * cpu_count 
    while all(usage == 0 for usage in cpu_usage):  
